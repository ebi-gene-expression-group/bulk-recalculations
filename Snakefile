...
rule rnaseq_qc:
    ...
    """
    output: "qc/{accession}-multiqc_report.html"
	params:
        quantification_dir=get_quantification_dir() 
    shell:
        """
        ...
        if [ -s "$expQuantDir/multiqc/star_salmon/multiqc_report.html" ] ; then
            rsync -avz $expQuantDir/multiqc/star_salmon/multiqc_report.html {output}
        else
            echo "$expQuantDir/multiqc/star_salmon/multiqc_report.html not found - skipping"
        fi
