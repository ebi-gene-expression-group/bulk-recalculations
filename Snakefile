...
def get_outputs():
    ...
    if 'percentile-ranks' in config['tool'] or config['tool']=="all-diff" and config['accession'] not in skip_accesion['skips']['percentile-ranks']:
        outputs.append(f"{config['accession']}-percentile-ranks.tsv")
    if 'differential-tracks' in config['tool'] or config['tool']=="all-diff" and config['accession'] not in skip_accesion['skips']['differential-tracks']:
    ...
    if 'baseline-tracks' in config['tool'] or config['tool']=="all-baseline" and config['accession'] not in skip_accesion['skips']['baseline-tracks']:
        ...
        outputs.extend(expand(config['accession']+".{a_id}.genes.expressions_{metric}.bedGraph",
                            a_id=get_assay_ids(),
                            metric=get_metrics()))
    if 'differential-gsea' in config['tool'] or config['tool']=="all-diff" and config['accession'] not in skip_accesion['skips']['differential-gsea']:
    ...
    if 'atlas-experiment-summary' in config['tool'] or 'all' in config['tool'] and config['accession'] not in skip_accesion['skips']['atlas_experiment_summary']:
        outputs.append(f"{config['accession']}-atlasExperimentSummary.Rdata")
    if 'baseline-heatmap' in config['tool'] or 'all-baseline' in config['tool'] and config['accession'] not in skip_accesion['skips']['baseline-heatmap']:
        outputs.extend(expand(f"{config['accession']}"+"-heatmap-{metric}.pdf", metric=get_metrics() ))
    if 'baseline-coexpression' in config['tool'] or 'all-baseline' in config['tool'] and config['accession'] not in skip_accesion['skips']['baseline-coexpression']:   
